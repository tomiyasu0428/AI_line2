...
def get_google_calendar_service(user_id: str):
    ...
    try:
        ...
        print(
            f"リフレッシュトークン: {token_info.get('refresh_token')[:10] if token_info.get('refresh_token') else 'なし'}"
        )
        ...
        if credentials.expired and credentials.refresh_token:
            ...
            token_info.update({"token": credentials.token, "refresh_token": credentials.refresh_token})
...
def find_event_by_query(service, event_query: Dict[str, Any]) -> Optional[str]:
    """クエリ条件に一致する予定を検索する"""
    try:
...
def update_calendar_event(user_id: str, event_query: Dict[str, Any], updated_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if "start_time" in updated_data:
            event["start"] = {"dateTime": updated_data["start_time"], "timeZone": "Asia/Tokyo"}
        if "end_time" in updated_data:
            event["end"] = {"dateTime": updated_data["end_time"], "timeZone": "Asia/Tokyo"}
